...
describe("userController", () => {
  ...
  describe("login", () => {
    it("should log in a user and return response", async () => {
      ...
      const mockResponse = { user: undefined, accessToken: undefined };
...
describe("refreshToken", () => {
  it("should refresh the access token", async () => {
    ...
    } as FastifyRequest<{ Body: { refreshToken: string } }>;
    ...
    (refreshTokenService as jest.Mock).mockResolvedValueOnce(mockResponse);

    await userController.refreshToken(request, reply);

    expect(refreshTokenService).toHaveBeenCalledWith(request.body.refreshToken);
    expect(reply.send).toHaveBeenCalledWith(mockResponse);
  });
